...
struct GitHubTool: ParsableCommand {
    ...
    struct ListRepos: ParsableCommand {
        ...
        func run() throws {
            ...
            let repos = repoLister.fetchRepositoriesSync(for: username, token: token)
            repoLister.displayRepositories(repos)
        }
    }
    ...
    struct DeleteRepo: ParsableCommand {
        ...
        func run() throws {
            let token = KeychainHelper.getToken(account: username) ?? ""
            repoLister.deleteRepository(owner: username, repo: repo, token: token, dryRun: dryRun) { result in
                switch result {
                case .success():
                    print("Repository \(repo) deleted successfully.")
                case .failure(let error):
                    print("Failed to delete repository \(repo): \(error.localizedDescription)")
                }
            }
        }
    }
